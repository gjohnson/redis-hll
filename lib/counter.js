...
Counter.prototype.add = function(item, fn){
  ...
  mm3.murmur32(item, function(err, hash){
    ...
    var key = self.key;
    var pos = hash % self.m;
    redis.getrange(key, pos, pos, function(err, existing){
      if (err) return fn(err);
      var value = Math.log(hash / self.m || 1) / Math.LN2 | 0;
      if (value <= existing.charCodeAt(0)) return fn();
      redis.setrange(key, pos, String.fromCharCode(value), fn);
    });
  });
};
